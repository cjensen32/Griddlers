# ${PROJECT_NAME} — Course Spec

**Version:** 0.7
**Course:** ${COURSE_NAME}
**Course position:** Chapters ${CHAPTER_FIRST}–${CHAPTER_LAST}. Chapter 1 (Java Foundations) is
complete at `${CH_01_LOCAL_REPO}` (`${CH_01_GH_REPO}`).
**Companion:** `nonogram-learning-guide-FINAL.md` — how to work. Read it first.
**Prerequisite:** `CHAPTER-1-REVIEW-FINAL.md` — what you already know, and how to prove you still
know it.
**Environment:** `project.env` — every name, path, link, and locked stylistic choice these
documents refer to by variable.


**Chapters 2 and 3 are expanded. Chapter 4 is a manifest. Chapters 5–${CHAPTER_LAST} are syllabus
lines.**
...
- One tier 1 file per production class, named `<ClassName>${TIER1_SUFFIX}`, mirroring the production
  package.
- Tier 2 files named for the flow — `Solve${TIER2_SUFFIX}`, not `SolverAndGridTest`. A flow test that
  spans four classes belongs in one file named after the behaviour, not split across four.
- Tier 3 files named `<MainClass>${TIER3_SUFFIX}`, one per runnable main.
...
Six files are copied rather than rebuilt — frozen reference, not exercises.
`${DOC_CHECKLIST}` lists them with their destinations.

### Lessons

**2.1 — The POM, from memory.** *(Review — Ch1 L0.)* Coordinates (`${GROUP_ID}` / `${ARTIFACT_ID}`),
`maven.compiler.release` `${JAVA_VERSION}`, `junit-jupiter` at test scope, a pinned Surefire. Write it before you look. Then diff against the
...
**2.3 — Git and the commit grammar.** *(Review — Ch1 `CONTEXT.md`.)* `git init`, then a `.gitignore`
written from scratch — that one has real decisions in it beyond `target/`, and they are worth
re-deriving.

Copy `${COMMIT_HOOK}` rather than rebuilding it, and enable it with
`git config core.hooksPath .githooks`. Its subtleties are shell subtleties, not Java ones: the
...
double-count. Read it until you can explain those three, then move on — retyping it teaches `sed`,
not the build system.

Chapter 1 used five commit groups; you own fewer surfaces here, so pick fewer and record them in
`project.env` as `COMMIT_GROUPS`.
...
`tools` subpackage under it — `${ENGINE_PACKAGE}` and `${TOOLS_PACKAGE}`. Nothing goes in the engine
yet. Write down the rule: **the engine never imports `${ENGINE_FORBIDDEN_IMPORTS}`, and never touches
`System.out`.** Everything that reads, writes, or prints lives in `tools`.
...
child's stdin so it sees EOF rather than blocking forever; a timeout of `${E2E_TIMEOUT_SECONDS}`
seconds with a forcible kill, so a hung child fails the test instead of hanging your build; and
decoding output as explicit UTF-8 rather than the platform default, which is the only reason your
Unicode glyphs survive Chapter 3.

Name it whatever you like and record that in `project.env` as `E2E_HARNESS_NAME`; this document
deliberately does not name it for you.
...
Glyphs: `${GLYPH_FILLED}` filled, `${GLYPH_EMPTY}` empty, `${GLYPH_MARKED}` marked. If your terminal
mangles Unicode, switch `GLYPH_MODE` to `ascii` and use the fallbacks. Pick a set now and record it in
`project.env` — changing it later breaks every string-match test you write between here and Chapter
${CHAPTER_LAST}.
...
**3.5 — Rulers.** *(New.)* A heavier separator every `${RULER_EVERY}`th row and column so you can count without
...
| Meaning | Unicode | ASCII fallback | Variable |
|---|---|---|---|
| Filled | `█` | `#` | `GLYPH_FILLED` / `GLYPH_FILLED_ASCII` |
| Empty | `·` | `.` | `GLYPH_EMPTY` / `GLYPH_EMPTY_ASCII` |
| Marked | `✕` | `X` | `GLYPH_MARKED` / `GLYPH_MARKED_ASCII` |
| Deduced this pass | `▓` | `+` | `GLYPH_DEDUCED` / `GLYPH_DEDUCED_ASCII` |

Choose in Chapter 3, set `GLYPH_MODE` in `project.env`, and do not revisit it.

# Appendix B — Repository layout

Paths below use `project.env` variables; the defaults are shown in the comment column.

```
${REPO_NAME}/
├─ project.env                      names, paths, links, locked choices
├─ ${JOURNAL_FILE}                       stuck/unstuck log and per-chapter gap counts
├─ pom.xml
├─ ${COMMIT_HOOK}
├─ ${CHECKSTYLE_CONFIG}
├─ ${LESSONS_DIR}/
...
│  │                                engine; none of ${ENGINE_FORBIDDEN_IMPORTS}, no printing
│  └─ tools/                        mains, file I/O, terminal game
├─ src/test/java/${PACKAGE_PATH}/
│                                   mirrors the above, file for file, plus shared harnesses
├─ ${PUZZLES_DIR}/                         generated JSON, committed
├─ ${WEB_DIR}/                             index.html, manifest.json, sw.js, icons/
└─ ${SPEC_HISTORY_DIR}/             this folder, as read-only provenance
```

The log is `${JOURNAL_FILE}`, not `NOTES.md`. The tracker already uses `NOTES.md` for Java version
...
All paths are relative to `${CH_01_LOCAL_REPO}`, under `src/test/java/com/connorjensen/jobtracker/`.
...
| 3 | `MainProcessTest.java` | `ProcessBuilder`, stdin closed, a timeout of `${E2E_TIMEOUT_SECONDS}`s, explicit UTF-8, exit code and exact stdout. |
