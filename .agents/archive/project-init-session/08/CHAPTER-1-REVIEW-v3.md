...
```
${CH_01_LOCAL_REPO}/${CH_01_LESSONS_DIR}/
```

If it is not checked out on this machine, clone it from `${CH_01_GH_REPO}`.
...
- [ ] `pom.xml` — coordinates (`${GROUP_ID}` / `${ARTIFACT_ID}`), `maven.compiler.release`
      `${JAVA_VERSION}`, `junit-jupiter` at test scope, pinned Surefire, compiler plugin.
- [ ] Checkstyle plugin bound to `validate`, pointing at `${CHECKSTYLE_CONFIG}`.
...
- [ ] `${COMMIT_HOOK}` and the `${COMMIT_COUNTER_PREFIX}###` commit grammar.
- [ ] `.editorconfig`.
- [ ] The `src/main/java` and `src/test/java` trees under `${PACKAGE_PATH}`.
...
- [ ] `${CHECKSTYLE_CONFIG}` (rebuilding a Checkstyle ruleset teaches nothing)
...
Under `${CH_01_LOCAL_REPO}/${CH_01_LESSONS_DIR}/`:
...
are relative to `${CH_01_LOCAL_REPO}`.
...
| A published behavioural contract | `${CH_01_LESSONS_DIR}/CAPSTONE.md` |
