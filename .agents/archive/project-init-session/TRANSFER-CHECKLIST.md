...
Status meanings in this working checklist:
...
Decisions made during unpacking:

- Environment configuration remains local-only and is excluded from the live repository and final
  provenance archive. Live documents render the needed values directly.
- The learner journal lives at `lessons/JOURNAL.md`.
- Only the project-level `README.md` lives at the repository root. Agent and lesson indexes stay in
  their owning directories.

---

The historical task descriptions below retain their original `${VARIABLE}` placeholders. The table
records the values used to render live documents; it does not require or authorize a tracked
environment file.
...
| `${PROJECT_LOCAL_REPO}`       | `/Users/connor/repos/JAVA/Griddlers`                       |
| `${PROJECT_GH_REPO}`          | `https://github.com/cjensen32/Griddlers.git` (private)     |
| `${LESSONS_DIR}`              | `lessons`                                                  |
...
One transfer-only exception: the footer's two relative links (`README.md`, `NOTES.md`) point at Chapter 1 files that do not exist here, so they were repointed at the same two files on `${CH_01_GH_REPO}`. The glossary text is untouched.
...
### [x] `${CH_01_LESSONS_DIR}/COURSE_STANDARDS.md` → `${LESSONS_DIR}/COURSE_STANDARDS.md`
...
### [x] `.editorconfig` → same path

Five lines. No changes. Nothing to learn by retyping it.

---

### [x] `${COMMIT_HOOK}` → same path  ·  copy **Agent**, read it **Learner**
...
### [x] Keep environment configuration local-only

The original plan put `project.env` at the repository root. The learner superseded that plan during
unpacking: environment files remain ignored local state, are not copied to the root, and will be
excluded from `${SPEC_HISTORY_DIR}`. Live lesson documents render factual values directly. Choices
made by later lessons are recorded by the lesson, code, and tests that make them.

Unpack outcome: `project.env` was carried out of this directory rather than archived with it, and
now sits gitignored at the repository root under the `*.env*` rule. Nothing tracked reads it. Move
it outside the repository or delete it if you would rather it not sit there — that is a **Learner**
call, and no document depends on the answer.

### [x] Everything else here → `${SPEC_HISTORY_DIR}/`
...
### [x] `.agents/CONTEXT.md` — first, before anything else
...
### [x] `.agents/{README,PROJECT,COURSE_MAINTENANCE}.md`

Committed in C003 with the root project entry point and learner journal.

### [x] `${LESSONS_DIR}/JOURNAL.md`
...
- [x] `${CH_01_GH_REPO}` is reachable **without your credentials** — verified `200` anonymously, so
      the answer key that `CHAPTER-1-REVIEW.md` and the spec's Appendix E both depend on is open to
      an agent in this repository. `${PROJECT_GH_REPO}` itself answers `404` anonymously and is
      private; that is this repository, so nothing depends on it being public.

- [x] No template variables or stale source filenames in the live documents:

      ```bash
      rg -n '\$\{|project\.env|nonogram-(spec|learning-guide)-FINAL|CHAPTER-1-REVIEW-FINAL' lessons
      ```

      Expected result: no output. The check covers `lessons/` only. `${SPEC_HISTORY_DIR}` is chat
      history and deliberately keeps its original placeholders and filenames, as this checklist's
      own preamble states — running the pattern there can never come back clean.
