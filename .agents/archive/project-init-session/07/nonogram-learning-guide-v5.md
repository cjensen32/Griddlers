...
- **Nonogram theory** — the Wikipedia article, and search "nonogram solver line solving algorithm"
  ...
  implementation.
- **Your own Chapter 1** — `ch01-GLOSSARY.md` for fast recall, `TESTING_STANDARDS.md` for test shape,
  `COURSE_STANDARDS.md` for conventions. The tracker repository holds the lessons behind them, and
  its three test files are your examples for the three tiers.
