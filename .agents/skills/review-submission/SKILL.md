...
7. **Write `commit_msg.txt` and stop before it lands.** One commit per cycle, carrying every surface the cycle touched, with the `CHANGES:` bullets `.agents/CONTEXT.md` owns doing the grouping that separate commits used to do. Name the exact files it carries, because the learner's working tree may hold unrelated work. Never fold the learner's own code, tests, or answers into it - those are their commit, authored by them, and the authorship split is the reason `git log` can still be read. The learner has final say on the message before it lands. Once it has, write the session artifact under `.agents/archive/session-artifacts/` as a log of the cycle: each step, and what changed at it.
