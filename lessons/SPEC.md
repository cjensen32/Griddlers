...
**Course position:** Chapters 2–14. Chapter 1 (Java Foundations) is complete in the local
`/Users/connor/repos/JAVA/job-application-tracker` checkout and the
[Job Application Tracker repository](https://github.com/cjensen32/job-application-tracker).
**Companion:** [Learning Guide](LEARNING_GUIDE.md) — how to work. Read it first.
**Prerequisite:** [Chapter 1 Review](CHAPTER-1-REVIEW.md) — what you already know, and how to prove
you still know it.

**Chapters 2 and 3 are expanded. Chapter 4 is a manifest. Chapters 5–14 are syllabus
lines.**
...
- One tier 1 file per production class, named `<ClassName>Test`, mirroring the production
  package.
- Tier 2 files named for the flow — `SolveFlowTest`, not `SolverAndGridTest`. A flow test that
...
**2.1 — The POM, from memory.** *(Review — Ch1 L0.)* Coordinates (`com.connorjensen` /
`griddlers`), `maven.compiler.release` `21`, `junit-jupiter` at test scope, a pinned Surefire. Write
it before you look. Then diff against the tracker's and write down every difference and why it
exists.
...
**2.3 — Git and the commit grammar.** *(Review — Ch1 `CONTEXT.md`.)* This repository and its initial
ignore rules already exist. Explain every current `.gitignore` entry, then add only the generated
state introduced by the build you create in Lessons 2.1 and 2.2.

Copy `.githooks/prepare-commit-msg` rather than rebuilding it, and enable it with
...
Chapter 1 used five commit groups; you own fewer surfaces here. Review the smaller set in
`COURSE_STANDARDS.md`, change it if needed before the first learner-code commit, and keep that file
as the learner-facing authority.
...
`tools` subpackage under it — `com.connorjensen.griddlers` and
`com.connorjensen.griddlers.tools`. Nothing goes in the engine yet. Write down the rule: **the engine
never imports `java.io`, `java.nio.file`, or `java.net`, and never touches `System.out`.** Everything
that reads, writes, or prints lives in `tools`.
...
child's stdin so it sees EOF rather than blocking forever; a timeout of `5`
...
Name it whatever you like. Its production-neutral helper and matching tests are the durable record;
this document deliberately does not name it for you.
...
Try both available glyph sets in the terminal you actually use. Pick Unicode (`█`, `·`, `✕`) or
ASCII (`#`, `.`, `X`), then make the exact-output renderer test the durable record of that decision.
Changing the set later breaks every string-match test you write between here and Chapter 14.
...
**3.5 — Rulers.** *(New.)* A heavier separator every `5`th row and column so you can count without
...
This is the intended end-state layout. Chapter 2 deliberately begins without the POM or source
trees shown below.
...
All paths are relative to `/Users/connor/repos/JAVA/job-application-tracker`, under
`src/test/java/com/connorjensen/jobtracker/`.
