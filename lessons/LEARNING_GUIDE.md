...
**Assumes:** Chapter 1 (Java Foundations) is complete. See
[Chapter 1 Review](CHAPTER-1-REVIEW.md).
...
**Keep a `JOURNAL.md`.** One line each time you get stuck and unstick yourself: what broke, what
...
It is `JOURNAL.md` rather than `NOTES.md` because the tracker already uses `NOTES.md` for Java
...
- **Nonogram theory** — the Wikipedia article, and search "nonogram solver line solving algorithm"
  ...
  implementation.
- **Your own Chapter 1** — `ch01-GLOSSARY.md` for fast recall, `TESTING_STANDARDS.md` for test
  shape, and `COURSE_STANDARDS.md` for conventions. The original lessons remain under
  `/Users/connor/repos/JAVA/job-application-tracker/lessons/ch01-java-foundations/`, and its three
  test files are your examples for the three tiers — see the spec's Appendix E.
