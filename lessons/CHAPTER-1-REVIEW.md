...
If it is not checked out on this machine, clone the
...
(`ch01-GLOSSARY.md`, `COURSE_STANDARDS.md`, `TESTING_STANDARDS.md`) cover most of it.
...
14. Why did `Status` never need a stable ordinal? What would have had to change if it were written
    to a file — and why is `ordinal()` the wrong thing to write?
...
26. Name the three test scopes from the Chapter 1 standards, and one thing each proves that the
    others cannot.
26a. Of your ten hand-written Chapter 1 tests, which tier was each one? Which tier did you write
    least of, and what kind of bug did that leave uncovered?
...
- [ ] `pom.xml` — coordinates (`com.connorjensen` / `griddlers`), `maven.compiler.release`
      `21`, `junit-jupiter` at test scope, pinned Surefire, compiler plugin.
...
introduces format stability as a constraint. Chapter 3 assigns every cell state an explicit external
code rather than deriving one from `ordinal()`; Chapters 9 and 12 depend on those stable codes.
...
**Owning the shape of your own code.** Related, and worth separating out. The Chapter 1 capstone
published its contract before the lessons, so the class names, method signatures, and exact output
were decided for you. That is a real cost even when the design is good: you never had to defend a
naming decision, discover that a class had two responsibilities, or find out the hard way that an
interface was in the wrong place. In this course nothing is named for you, and the corresponding
skill — deciding shape, then living with it — is one you have not practised.

**Deleting things.** Nothing in the tracker was ever removed once it worked. Test maintenance —
rewriting, restructuring, and deleting tests you have outgrown — does not appear anywhere in
Chapter 1, and it starts in Lesson 3.5 here.
...
Working reference implementations worth reading *after* you have written your own version. All paths
are relative to `/Users/connor/repos/JAVA/job-application-tracker`.
